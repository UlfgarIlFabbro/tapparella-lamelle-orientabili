...
from .const import ENTITY_STORE, STATE_OPEN, STATE_CLOSED, STATE_TILT, ip_slug

_LOGGER = logging.getLogger(__name__)

STATE_LABELS = {STATE_OPEN: "Aperta", STATE_CLOSED: "Chiusa", STATE_TILT: "Lamelle aperte"}
BUTTON_LABELS = {STATE_OPEN: "Su", STATE_CLOSED: "Giù", STATE_TILT: "Lamelle"}


class TapparellaStatoSensor(SensorEntity):
    _attr_icon = "mdi:blinds"

    def __init__(self, entry, cover_entity):
        self._cover = cover_entity
        self._attr_name = f"{entry.data['name']} Stato"
        self._attr_unique_id = f"tlo_{ip_slug(entry.data['ip'])}_stato"

    @property
    def available(self): return self._cover._available

    @property
    def native_value(self): return STATE_LABELS.get(self._cover._state, "Sconosciuto")

    @property
    def extra_state_attributes(self): return {"stato_raw": self._cover._state}
...
class TapparellaUltimoPulsanteSensor(SensorEntity):
    ...
    @property
    def native_value(self): return BUTTON_LABELS.get(self._cover._state, "Nessuno")
...
class TapparellaIPSensor(SensorEntity):
    ...
    @property
    def native_value(self): return self._entry.data.get("ip")
...
async def async_setup_entry(
    ...
    if cover_entity is not None:
        entities += [TapparellaStatoSensor(entry, cover_entity), TapparellaUltimoPulsanteSensor(entry, cover_entity)]
    else:
