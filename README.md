...
### Novità della versione 1.4.2

- Aggiunta sincronizzazione periodica con `Shelly.GetStatus`.
- Lo stato `open` dello Shelly sincronizza TLO in **OPEN**.
- Lo stato `stopped` dello Shelly sincronizza TLO in **CLOSED**.
- Lo stato `closed` dello Shelly viene trattato come **ambiguo** e non sovrascrive automaticamente TLO, perché Cherubini lo utilizza anche per la posizione con lamelle aperte (TILT).
- Mantenuta la logica delle transizioni del comando fisico GIÙ: da OPEN/CLOSED verso TILT e da TILT verso CLOSED.
- Ridotto il rischio che Home Assistant mantenga uno stato errato dopo la perdita di un webhook, utilizzando il polling per gli stati Shelly non ambigui.
